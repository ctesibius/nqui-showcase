...
The basic preview matches [bar](/docs/nqchart/bar-chart) and [line](/docs/nqchart/line-chart): twelve months, `desktop` bars, `mobile` line, same emerald/rose tokens, and the same default **3px** top corner radius on bars (`barRadius` / `DEFAULT_BAR_RADIUS`, nqui `--radius-sm`).
...
Use `prepareParetoData()` from [Chart recipes](/docs/nqchart/chart-recipes), then bar + line on `<NQComposedChart />` with left/right Y axes. Pareto uses its own category data (not the traffic dataset above).
...
<ApiTable>
  ...
  <ApiRow name="onMarkClick" type="(event: NQMarkEvent) => void">
    Fires when a mark is clicked (or Enter after keyboard focus). `category` is the raw datum field.
  </ApiRow>
  <ApiRow name="onBrushChange" type="(range: ChartBrushRange) => void">
    Fires while the brush range changes. Prefer this over the older `NQBrushRange` name.
  </ApiRow>
  <ApiRow name="onChartReady" type="(instance: EChartsType) => void">
    Escape hatch — unsupported ECharts instance access.
  </ApiRow>
  <ApiRow name="chartRef" type="Ref&lt;ChartHandle | null&gt;">
    Imperative handle: `getInstance()`, `toDataURL()` (PNG with themed background).
  </ApiRow>
...
<ApiTable>
  <ApiRow name="dataKey" type="string" required>
    The series key. Must exist on both the data rows and the chart `config`. May be shared with `<Area />` for a fill-under-line: the legend shows one row and `onMarkClick.seriesKey` stays this key.
  </ApiRow>
  <ApiRow name="variant" type='"solid" | "dashed" | "dotted" | "points"' default='"solid"'>
    `"dashed"` / `"dotted"` set the stroke dash. `"points"` draws markers only (box-plot median ticks).
  </ApiRow>
...
<ApiTable>
  ...
  <ApiRow name="selected" type="string | null">
    Controlled single-select series key. Pass with `onSelectChange`.
  </ApiRow>
  <ApiRow name="onSelectChange" type="(selected: string | null) => void">
    Controlled legend selection callback.
  </ApiRow>
</ApiTable>
