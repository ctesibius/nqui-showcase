...
links:
  github: https://github.com/ctesibius/nqchart/blob/main/src/registry/charts/line-chart.tsx
...
<CodeTabs>
  ...
  <TabsPanel value="cli">
    <CliBlock commands={["@nqchart/line-chart"]} />
  </TabsPanel>
    ...
    <Steps>
      ...
      <Step>
        ...
        <StepContent>
          <ComponentSource
            name="line-chart"
            title="components/nqchart/charts/line-chart.tsx"
          />
        </StepContent>
      </Step>
       <Step>
...
The line chart is composible. `<NQLineChart>` is the container, and you compose only the parts you need — `<Grid>`, `<XAxis>`, `<YAxis>`, `<Legend>`, `<Tooltip>`, and one or more `<Line>` — as its children. Each `<Line>` carries its own `variant` (`solid` / `dashed` / `dotted`), `curveType`, and `isClickable`, so a single chart can mix stroke styles and make only some series interactive.
...
import {
  ...
  Tooltip,
  Legend,
} from "@/components/nqchart/charts/line-chart";
```
...
<NQLineChart data={data} config={chartConfig} xDataKey="month">
  <Grid />
  <XAxis dataKey="month" />
  <Legend />
  <Tooltip />
...
Uses the same `desktop` / `mobile` traffic dataset as the [bar](/docs/nqchart/bar-chart) and [composed](/docs/nqchart/composed-chart) basic previews.
...
<Alert > 
  <AlertContent >
    Pass `isLoading` on `<NQLineChart>` while data is fetching. The chart shows a loading overlay on top of the composed chart area.
  </AlertContent>
</Alert>
...
<ApiHeading>NQLineChart</ApiHeading>
...
<ApiTable>
  ...
  <ApiRow name="hoverFocus" type="boolean" default="true">
    Hover dim: focused mark full opacity, siblings at `0.2`. `false` keeps the tooltip and disables sibling fade. See [Hover Focus](/docs/nqchart/hover-focus).
  </ApiRow>
  <ApiRow name="onMarkClick" type="(event: NQMarkEvent) => void">
    Fires when a mark is clicked or activated with keyboard Enter. Payload includes raw `category`, `seriesKey` (config dataKey), `datum`, `index`, and modifiers.
  </ApiRow>
  <ApiRow name="onBrushChange" type="(range: ChartBrushRange) => void">
    Fires while the brush range changes. Prefer this over the older `NQBrushRange` alias in new code.
  </ApiRow>
  <ApiRow name="onChartReady" type="(instance: EChartsType) => void">
    Escape hatch when the ECharts instance is ready. Prefer typed props when possible.
  </ApiRow>
  <ApiRow name="chartRef" type="Ref&lt;ChartHandle | null&gt;">
    Imperative handle with `toDataURL()` (themed PNG) and `getInstance()`.
  </ApiRow>
  <ApiRow name="isEmpty" type="boolean">
    Force the empty plate. Default: derived from `data.length === 0`.
  </ApiRow>
  <ApiRow name="emptyState" type="ReactNode">
    Custom empty plate content.
  </ApiRow>
  <ApiRow name="error" type="ReactNode">
    Error plate — visually distinct from empty; replaces the chart.
  </ApiRow>
  <ApiRow name="a11yTable" type="boolean" default="true">
  ...
  <ApiRow name="chartProps" type="ComponentProps<typeof LineChart>">
    Additional ECharts options merged into the compiled `option` by NQChart. Read the <Link href="https://echarts.apache.org/en/option.html#series-line" _blank>ECharts LineChart documentation</Link> for available props.
  </ApiRow>
</ApiTable>
...
<ApiTable>
  ...
  <ApiRow name="variant" type='"solid" | "dashed" | "dotted"' default='"solid"'>
    Stroke dash. Use `"dashed"` to tell a rolling mean apart from a parent series in the same colour.
  </ApiRow>
