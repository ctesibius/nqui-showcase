...
### Monospace (fold + hover)

Monochrome bars **load expanded**, **fold to thin** on intro, then **expand on hover**. Use `<Bar variant="monospace" />` (also on [Bar blocks](/docs/nqchart/bar-chart/blocks)).

<ComponentPreview className="mb-4" title="variant='monospace'" name="monospace-bar-chart" />
...
Bin numeric samples with `binForHistogram()` from [Chart recipes](/docs/nqchart/chart-recipes), then render with `variant="histogram"` on `<NQBarChart />` (touching bins, square corners).
...
<ApiTable>
  ...
  <ApiRow name="variant" type='"default" | "hatched" | "duotone" | "duotone-reverse" | "gradient" | "stripped" | "monospace" | "hover-trace"' default='"default"'>
    Fill / interaction style for this series. `monospace` folds bars after intro and expands on hover; `hover-trace` draws a category focus guide.
  </ApiRow>
