...
<ComponentPreview className="mb-0" title="Mark click + budget cap" name="ex-mark-click-budget-chart"  />
...
The bar chart is composable. `<NQBarChart>` is the container, and you compose only the parts you need — `<Grid>`, `<XAxis>`, `<YAxis>`, `<Legend>`, `<Tooltip>`, and one or more `<Bar>` — as its children. Each `<Bar>` carries its own `variant`, `radius`, `glowing`, `bufferBar`, and `isClickable`, so a single chart can mix fill styles and make only some series interactive.

Category axis labels use collision hiding (`hideOverlap`) so dense previews stay readable. The optional brush footer shows range labels at rest; for `layout="horizontal"`, the brush mini-preview maps categories along X so filtering lines up with the window.
...
Isometric bars with beveled faces and a highlighted peak — see [Bar blocks](/docs/nqchart/bar-chart/blocks) for the full block.

<ComponentPreview title="Isometric 3D bar" name="isometric-bar-chart"  />

### Histogram

Bin numeric samples with `binForHistogram()` from [Chart recipes](/docs/nqchart/chart-recipes), then render with `variant="histogram"` on `<NQBarChart />` (touching bins, square corners). Long bin labels truncate and hide overlaps when the plot is narrow — widen the card to show more ticks.
...
<ApiTable>
  ...
  <ApiRow name="showBrush" type="boolean" default="false">
    When enabled, displays a brush control below the chart for selecting a category range. Handle labels stay visible at rest. Horizontal layouts use a vertical category mini-preview so the window matches category bands.
  </ApiRow>
