---
title: "Dependencies"
description: "A dependency is a stored relationship between two tasks: who waits on whom, how the ends connect, and optional lag. The line on the chart is a view of that data"
---
...
| **Type** | FS / SS / FF / SF — see [Dependency types](/docs/nqgantt/dependency-types) |
...
4. Changing lag updates the chip (e.g. `+3d`). With [Auto-schedule](/docs/nqgantt/auto-schedule) on, successors re-solve immediately; with it off, only the link metadata changes until you move a bar later with schedule on.
...
In this showcase, [Practice in Gantt lab](/docs/nqgantt/practice-in-gantt-lab) / `RoadmapGantt` keeps an internal dependency list for demos.

## When not to

- Do not create links you are not willing to honor in planning meetings.
- Do not configure type “for the whole chart”—type is **per link**. See [Design philosophy](/docs/nqgantt/philosophy).
