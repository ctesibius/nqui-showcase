...
Current line: **0.3.2**. Independent of `@nqlib/nqui` — install any nqui version (or none).
...
| `hoverFocus` | every `NQ*Chart` root (default on) | Sibling dim on hover. `false` keeps tooltip, skips fade |
