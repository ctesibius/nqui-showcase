...
This site's Appearance picker writes brand hue to `--primary` only. `--accent` and `--interactive` stay muted so hover/menu wash is not a primary fill. Try it at [/settings/appearance](/settings/appearance).
