 ...
 *
 * Also picks up standalone `src/registry/examples/ex-*.tsx` files that exist
 * in becocharts but are missing from a stale registry.json (so docs MDX
 * `<ComponentPreview>` names don't 404). Then audits content/docs names
 * against the catalog + docs-extra.
 *
 * Usage:
...
const FAMILY_RULES = [
  [/area/, "area"],
  [/bar|histogram|bullet|monospace|hover-trace|grid-bar|isometric|mark-click/, "bar"],
  [/line/, "line"],
...
const SHARED_EXAMPLE_FILES = new Set([
  "example-shared.ts",
  "example-datasets.ts",
  "workload-demo-data.ts",
  "chart-tokens.ts",
  "ex-doc-charts.tsx",
  "ex-ui-charts.tsx",
]);

/** Standalone `ex-*.tsx` files that exist in becocharts but are missing from a stale registry.json. */
function listOrphanExampleFiles(becoRoot, knownIds) {
  const examplesDir = path.join(becoRoot, "src/registry/examples");
  if (!existsSync(examplesDir)) return [];
  return readdirSync(examplesDir)
    .filter((f) => /^ex-.+\.tsx$/.test(f) && !SHARED_EXAMPLE_FILES.has(f))
    .map((f) => {
      const id = f.replace(/\.tsx$/, "");
      return { id, absSrc: path.join(examplesDir, f) };
    })
    .filter(({ id, absSrc }) => !knownIds.has(id) && existsSync(absSrc));
}

function listDocsExtraIds() {
  const extraPath = path.join(OUT_DIR, "docs-extra-entries.ts");
  if (!existsSync(extraPath)) return [];
  return [...readFileSync(extraPath, "utf8").matchAll(/id:\s*"([^"]+)"/g)].map(
    (m) => m[1],
  );
}

function listMdxPreviewNames() {
  ...
  const walk = (d) => {
    for (const ent of readdirSync(d, { withFileTypes: true })) {
      ...
      if (ent.isDirectory()) walk(p);
        ...
        for (const m of src.matchAll(/<ComponentPreview\b[^>]*\bname="([^"]+)"/g)) {
          names.add(m[1]);
        }
      }
    }
...
function auditDocsPreviews(catalogIds) {
  const known = new Set([...catalogIds, ...listDocsExtraIds()]);
  const missing = listMdxPreviewNames().filter((id) => !known.has(id));
  if (missing.length) {
    console.error(
      `[sync-nqchart-catalog] MDX <ComponentPreview> names missing from catalog:`,
    );
...
`,
  ...
  // registry.json is generated and can lag `__index__.tsx` / `registry-example.ts`.
  // Pick up standalone example files so docs previews don't 404.
  const knownIds = new Set(entries.map((e) => e.id));
  for (const orphan of listOrphanExampleFiles(BECO, knownIds)) {
    const adapterFile = writeAdapter(orphan.absSrc);
    const source = readFileSync(orphan.absSrc, "utf8");
    const exportName = firstExportName(source);
    if (!exportName) {
      console.warn(`[sync-nqchart-catalog] no export for orphan ${orphan.id}`);
      continue;
    }
    const family = inferFamily(orphan.id);
    const chartImport = source.match(
      /from\s+"@\/registry\/charts\/([^"]+)"/,
    )?.[1];
    console.info(
      `[sync-nqchart-catalog] orphan example (not in registry.json): ${orphan.id}`,
    );
    entries.push({
      id: orphan.id,
      name: titleize(orphan.id),
      exportName,
      adapterFile,
      family,
      category: inferCategory(orphan.id, family),
      component: chartImport ?? (family === "bar" ? "bar-chart" : family),
    });
...
`,
  ...
  auditDocsPreviews(entries.map((e) => e.id));
  console.info(
